...
printf "${BLUE}Packckaging TaskSharing chaincode:${NC}\n"
sleep 1
# Org1 env var
...
# package
peer lifecycle chaincode package taskSharing.tar.gz --path ../../../../eclipse-workspace/TaskSharingChaincode_v0/lib/build/install/lib --lang java --label taskSharingV0
cc_pkg=$(peer lifecycle chaincode calculatepackageid taskSharing.tar.gz)
printf "${GREEN}Chaincode was packaged${NC}\n\n"
sleep 1

# install
printf "${BLUE}Installing chaincode in Org1:${NC}\n"
peer lifecycle chaincode install taskSharing.tar.gz --peerAddresses localhost:7051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE
...
peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.tasksharingnet.com --tls --cafile $ORDERER_CA --channelID globalchannel --name TaskSharing --version 0.1 --init-required --package-id $cc_pkg --sequence 1
printf "${GREEN}Chaincode approved for Org1${NC}\n\n"
sleep 1

printf "${BLUE}Checking chaincode approvals:${NC}\n"
sleep 1
peer lifecycle chaincode checkcommitreadiness --channelID globalchannel --name TaskSharing --version 0.1 --sequence 1 --output json --init-required
...
peer lifecycle chaincode install taskSharing.tar.gz --peerAddresses localhost:9051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE
...
peer lifecycle chaincode install taskSharing.tar.gz --peerAddresses localhost:10051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE
...
peer lifecycle chaincode install taskSharing.tar.gz --peerAddresses localhost:12051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE
...
# committ
printf "${BLUE}Committing TaskSHaring Chaincode to globalchannel:${NC}\n"
peer lifecycle chaincode commit -o localhost:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA --channelID globalchannel --name TaskSharing --peerAddresses localhost:7051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE_ORG1 --peerAddresses localhost:9051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE_ORG2 --peerAddresses localhost:10051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE_ORG3 --peerAddresses localhost:11051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE_ORG4 --peerAddresses localhost:12051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE_ORG5 --version 0.1 --sequence 1 --init-required
